...
def parse_multiqc_files(fastq_folder_name):
    ...
    # Divide the standard deviation by a million
    std_deviation_millions = std_deviation / 1e6
    std_deviation_millions = round(std_deviation_millions, 2)
    ...
    return [std_deviation_millions, cv_percentage, undertermined_read_percentage, distribution_string, count_total_reads]
...
def parse_fastq_stats_folder(fastq_folder_name):
    ...
    distribution_string = '-'.join(map(str, top_percentages))

    # Find PhiX in output
    desired_pattern_no_plus = 'G'
    desired_value_paired_end = 'GGGGGGGGGG+AGATCTCGGT'
    for barcode, value in unknown_barcodes_array:
        if '+' in barcode:
            if barcode == desired_value_paired_end:
                print(f"Found the pair with '+': {barcode}: {value}")
                phix_output = value
                phix_barcode = barcode
                break  
        else:
            if all(char == desired_pattern_no_plus for char in barcode):
                print(f"Found the pair without '+': {barcode}: {value}")
                phix_output = value
                phix_barcode = barcode
                break  
    else:
        phix_output = None
        phix_barcode = None
        print("No matching pair found for PhiX output")

    return [unknown_barcodes_array[0][0], distribution_string, main_unknown_barcode_percentage, phix_output, phix_barcode]
